package com.boot.bucket.common.config;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.serializer.SerializeConfig;
import com.alibaba.fastjson.serializer.SerializerFeature;
import com.boot.bucket.common.contant.RedisKeyContant;
import com.boot.bucket.common.realm.CustomRealm;
import com.boot.bucket.util.FastJsonUtil;
...
public class ShiroConfig {
    ...
    public static final String PREFIX = "bucket.shiro";
     ...
     * 安全管理器
     *
     * @param customRealm                自定义 realm {@link #userRealm(CacheManager, HashedCredentialsMatcher)}
     ...
     */
    @Bean
    public SecurityManager securityManager(CustomRealm customRealm, DefaultWebSessionManager shiroRedisSessionManager) {
     ...
     * 用户Realm
     * <p>
     * SQL已经实现缓存 {@link com.boot.bucket.mapper.UserMapper}
     * todo 此处未实现缓冲
     * shiro默认缓存这里还有点坑需要填
     *
     * @return
     */
    @Bean
    public CustomRealm userRealm(CacheManager shiroRedisCacheManager, HashedCredentialsMatcher hashedCredentialsMatcher) {
        ...
        customRealm.setCacheManager(shiroRedisCacheManager);
        return customRealm;
    }
...
//            response.getWriter().write(ResponseUtil.resultFailed(ResultEnum.用户登陆过期));
            ...
            response.getWriter().write(JSON.toJSONString(ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(null), serializeConfig, SerializerFeature.WriteMapNullValue));
     ...
     * <p>
     * 前后端分离项目，直接获取url进行匹配，后台配置的权限的值就是请求路径 {@link CustomRealm#doGetAuthorizationInfo(PrincipalCollection)}
     *
     * @author seer
...
//            response.getWriter().write(ResponseUtil.resultFailed(ResultEnum.权限不足));
            SerializeConfig serializeConfig = FastJsonUtil.getSerializeConfig();
            response.getWriter().write(JSON.toJSONString(ResponseEntity.status(HttpStatus.FORBIDDEN).body(null), serializeConfig, SerializerFeature.WriteMapNullValue));
            return false;
        }
    ...
    class WebLogoutFilter extends LogoutFilter {
        ...
        protected boolean preHandle(ServletRequest request, ServletResponse response) throws Exception {
            response.setContentType("application/json");
            SerializeConfig serializeConfig = FastJsonUtil.getSerializeConfig();
            response.getWriter().write(JSON.toJSONString(ResponseEntity.ok(), serializeConfig, SerializerFeature.WriteMapNullValue));
            Subject subject = getSubject(request, response);
